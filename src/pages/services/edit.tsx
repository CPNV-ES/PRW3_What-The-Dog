...
const Service: NextLayoutPage = () => {
  ...
  function handleEditSubmit(data: z.infer<typeof ServiceSchema>) {
  }
  ...
  return (
    <Container>
      <Form className="h-full px-5 pt-8 text-white bg-cyan-700" schema={ServiceSchema} onSubmit={handleEditSubmit}>
        <h1 className='mb-5'>{`Edition d'un service`}</h1>
        ...
        <button className="w-full mt-5 border border-white" type="submit">Modifier</button>
      </Form>
      <Form className="h-full px-5 pb-8 text-white bg-cyan-700" onSubmit={handleDeleteSubmit}>
        <button className="w-full mt-5 bg-red-500 border border-white" type="submit">Supprimer le service</button>
      </Form>
    </Container>
  )
}
